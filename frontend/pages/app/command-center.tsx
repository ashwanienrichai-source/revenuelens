...
import { Upload, Play, Download, Loader2, CheckCircle, AlertCircle, BarChart3, TrendingUp, Users, DollarSign, Layers, Target, ChevronRight, RefreshCw, Lock, ArrowUp, ArrowDown, Minus, FileText, AlertTriangle, Home, ChevronDown, ChevronUp } from 'lucide-react'
...
const ENGINE_CONFIG = {
  ...
  cohort: {
    label:'Cohort Analytics', desc:'Size, Percentile & Revenue cohort segmentation', icon:Layers,
    required:[
      {key:'customer',label:'Customer / Account',kw:['customer_id','customer','client','account']},
...
function buildAutoMap(eng, cols) {
  const map={}; const cfg=ENGINE_CONFIG[eng]; if(!cfg)return map
...
export default function CommandCenter() {
  ...
  async function uploadFile(f) {
    ...
    try {
      ...
      if(data.is_acv)setEngine('acv'); else if(data.is_bridge_output)setEngine('mrr')
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      if(engine==='cohort'){
        fd.append('metric',fieldMap.revenue||'');fd.append('customer_col',fieldMap.customer||'')
        fd.append('date_col',fieldMap.date||'');fd.append('fiscal_col',fieldMap.fiscal||'None')
        fd.append('cohort_types',JSON.stringify(['SG','PC','RC']))
        const dims=['product','region','channel'].map(k=>fieldMap[k]).filter(Boolean)
        fd.append('individual_cols',JSON.stringify(dims));fd.append('hierarchies',JSON.stringify([]))
        fd.append('period_filter','all');fd.append('selected_fiscal_year','')
        const {data}=await axios.post(`${API}/api/cohort/analyze`,fd,{timeout:120000})
        setResults({...data,_engine:'cohort'});setActiveTab('summary')
      } else {
        fd.append('tool_type',engine==='acv'?'ACV':'MRR');fd.append('revenue_unit',revenueUnit)
        fd.append('lookbacks',JSON.stringify(lookbacks))
        const dims=['product','region','channel','vintage'].map(k=>fieldMap[k]).filter(Boolean)
        fd.append('dimension_cols',JSON.stringify(dims));fd.append('modules',JSON.stringify(['bridge','top_movers','top_customers','kpi_matrix','output']))
        fd.append('year_filter',yearFilter!=='All'?yearFilter:'');fd.append('period_type',periodType)
        fd.append('customer_col',fieldMap.customer||'Customer_ID');fd.append('n_movers','30');fd.append('n_customers','10')
    ...
    <div className="flex h-screen bg-[#F6F7F9] overflow-hidden" style={{fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <aside className="w-72 bg-white border-r border-ink-200 flex flex-col overflow-hidden flex-shrink-0">
        ...
        <div className="px-3 py-2 border-b border-ink-100 bg-ink-50/50 flex-shrink-0">
          <div className="space-y-0.5">
            ...
            <Step n={4} label="Analyze Metrics" done={!!results} active={step3&&!results}/>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-4 border-b border-ink-100">
            ...
            {uploadErr&&<div className="mb-2 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-[10px] flex items-start gap-1.5"><AlertCircle size={11} className="mt-0.5 flex-shrink-0"/><span>{uploadErr}</span></div>}
          ...
          {step2&&cfg&&(
            <div className="p-4 border-b border-ink-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[9px] text-ink-400">{cfg.required.filter(f=>!!fieldMap[f.key]).length}/{cfg.required.length} required</span>
              </div>
              <div className="rounded-xl border border-ink-200 overflow-hidden mb-2">
                <div className="px-3 py-1.5 bg-ink-50 border-b border-ink-100"><span className="text-[9px] font-700 text-ink-500 uppercase tracking-widest">Required Fields</span></div>
        ...
        <div className="p-4 border-t border-ink-100 flex-shrink-0">
          {runErr&&<div className="mb-2 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-[10px] flex items-start gap-1.5"><AlertCircle size={10} className="mt-0.5 flex-shrink-0"/>{runErr}</div>}
          <button onClick={runAnalysis} disabled={!step1||!step2||running} title={!step1?'Upload a file first':!step2?'Select an engine':!step3?'Map all required fields':''} className={`w-full flex items-center justify-center gap-2 font-700 text-sm py-3 rounded-xl transition-all ${canRun?'bg-brand-600 text-white hover:bg-brand-700':step3?'bg-brand-400 text-white cursor-not-allowed':'bg-ink-200 text-ink-400 cursor-not-allowed'}`}>
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        <header className="h-14 bg-white border-b border-ink-200 flex items-center px-6 gap-4 flex-shrink-0">
          <div><div className="text-sm font-700 text-ink-900">Subscription Analytics</div><div className="text-[10px] text-ink-400">Real-time retention and revenue analytics</div></div>
          ...
          {results&&(
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 p-0.5 bg-ink-100 rounded-lg">
                {lookbacks.map(lb=><button key={lb} onClick={()=>setSelLb(lb)} className={`px-2.5 py-1 rounded-md text-[11px] font-600 transition-all flex items-center gap-1 ${selLb===lb?'bg-white shadow-sm text-ink-900':'text-ink-500'}`}><TrendingUp size={10}/> {lb}M</button>)}
              </div>
              <button onClick={()=>{setResults(null);setFile(null);setColumns([]);setEngine(null);setFieldMap({})}} className="p-1.5 text-ink-400 hover:text-ink-700 rounded-lg hover:bg-ink-100 transition-all"><RefreshCw size={14}/></button>
              {isAdmin?<button onClick={downloadCSV} className="flex items-center gap-1.5 bg-brand-600 text-white text-[11px] font-700 px-3 py-1.5 rounded-lg hover:bg-brand-700 transition-all"><Download size={11}/> Export</button>
              :<button onClick={()=>router.push('/dashboard/upgrade')} className="flex items-center gap-1.5 text-[11px] font-600 text-ink-500 border border-ink-200 px-3 py-1.5 rounded-lg hover:bg-ink-50 transition-all"><Lock size={11}/> Upgrade</button>}
            </div>
          )}
        </header>
        ...
        {!results?(
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-lg">
              ...
              <p className="text-ink-500 text-sm mb-6 leading-relaxed">Upload your Alteryx bridge output or revenue CSV, select your analytics engine, map your fields, and click Analyze Metrics.</p>
              <div className="grid grid-cols-3 gap-3">
                {[{icon:TrendingUp,label:'Revenue Bridge',desc:'Waterfall bridge with classifications'},{icon:Target,label:'Top Movers',desc:'Biggest churners, new logos, upsells'},{icon:Users,label:'Top Customers',desc:'Top N customers by ending ARR'}].map((m,i)=>(
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-ink-200 bg-white">
              <div className="grid grid-cols-6 gap-3">
                ...
                <KpiCard label="Rows Analyzed"   value={(results.metadata?.row_count||results.summary?.rows_analyzed||0).toLocaleString()} icon={Users}/>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-auto p-6">
              ...
              {activeTab==='retention'&&(
                <div className="space-y-5">
                  ...
                  <div className="bg-white rounded-xl border border-ink-200 overflow-hidden">
                    <table className="w-full text-sm">
                      <thead><tr className="border-b border-ink-100 bg-ink-50">{['Period','Beginning','Ending','New Logo','Upsell','Downsell','Churn','GRR','NRR'].map(h=><th key={h} className="px-4 py-2.5 text-left font-700 text-ink-500 text-xs uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
                      <tbody>
                        {kpiRows.map((row,i)=>(
              ...
              {activeTab==='kpi_matrix'&&(
                <div className="bg-white rounded-xl border border-ink-200 overflow-hidden">
                  ...
                  <div className="overflow-x-auto"><table className="w-full text-sm">
                    <thead><tr className="border-b border-ink-100">{['Period','Beginning','Ending','New Logo','Upsell','Downsell','Churn','Cross-sell','GRR','NRR'].map(h=><th key={h} className="px-4 py-2.5 text-left font-700 text-ink-500 text-[10px] uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
