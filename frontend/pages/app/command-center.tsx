...
import { Upload, Play, Download, Loader2, CheckCircle, AlertCircle, BarChart3, TrendingUp, Users, DollarSign, Layers, Target, RefreshCw, Lock, ArrowUp, ArrowDown, FileText, Home, ChevronDown, ChevronUp, Plus, X } from 'lucide-react'
...
const ENGINE_CONFIG = {
  mrr: {
    label:'MRR / ARR Analytics', desc:'Alteryx bridge output — waterfall, retention, movers', icon:TrendingUp,
    required:[
  ...
  acv: {
    label:'ACV / Contract Analytics', desc:'Contract-based bridge with renewal rates', icon:FileText,
    required:[
      {key:'customer',label:'Customer / Account',kw:['customer_id','customer','client','account']},
      {key:'contractStart',label:'Contract Start Date',kw:['contract_start','start_date','startdate']},
      {key:'contractEnd',label:'Contract End Date',kw:['contract_end','end_date','expiry','enddate']},
  ...
  cohort: {
    label:'Cohort Analytics', desc:'Retention heatmap, revenue cohorts, segmentation', icon:Layers,
    required:[
      {key:'customer',label:'Customer / Account',kw:['customer_id','customer','client','account']},
...
const COHORT_PERIODS = ['Monthly','Quarterly','Yearly']
const GROUPBY_OPTIONS = ['Product','Region','Channel','Industry','Market','Country','State']

// ── Auto-detect ───────────────────────────────────────────────────────
function autoDetect(cols, kws) {
  const n = s=>s.toLowerCase().replace(/[^a-z0-9]/g,'')
  const nkws = kws.map(n)
  return cols.find(c=>nkws.includes(n(c))) || cols.find(c=>nkws.some(k=>n(c).includes(k)||k.includes(n(c)))) || ''
}
function buildAutoMap(eng, cols) {
  const map={}, cfg=ENGINE_CONFIG[eng]; if(!cfg) return map
  ;[...cfg.required,...cfg.optional,...cfg.kpiLens].forEach(f=>{const v=autoDetect(cols,f.kw);if(v)map[f.key]=v})
  return map
}
...
function UploadTimer({active}) {
  ...
  return (
    <div className="mt-2 px-1">
      <div className="flex justify-between mb-1"><span className="text-[10px] text-ink-500">{msg}</span><span className="text-[10px] font-700 text-brand-600">{s}s</span></div>
      <div className="h-1 bg-ink-200 rounded-full overflow-hidden"><div className="h-full bg-brand-500 rounded-full transition-all duration-1000" style={{width:`${pct}%`}}/></div>
...
function Step({n,label,done,active}) {
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${active?'bg-brand-50 border border-brand-200':''}`}>
      <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-700 flex-shrink-0 ${done?'bg-green-500 text-white':active?'bg-brand-600 text-white':'bg-ink-200 text-ink-500'}`}>{done?'✓':n}</div>
...
function FieldRow({label,required,value,columns,onChange,showError}) {
  return (
    <div className="grid grid-cols-2 gap-2 px-3 py-2 border-b border-ink-50 last:border-0">
      <div className="flex items-center gap-1"><span className="text-[11px] text-ink-700 truncate">{label}</span>{required&&<span className="text-red-400 text-[10px]">*</span>}</div>
      <select value={value} onChange={e=>onChange(e.target.value)} className={`text-[10px] border rounded px-1.5 py-1 bg-white text-ink-800 outline-none w-full ${showError&&!value?'border-red-300 bg-red-50':value?'border-green-300':'border-ink-200'}`}>
...
// ── Heatmap component — blank for zero/null ───────────────────────────
function CohortHeatmap({data, title, isPercent}) {
  ...
  const color = (v) => {
    ...
    if(isPercent) {
      ...
      // Revenue heatmap — relative intensity
      return '#3D5EFF'
    }
  }
  const textColor = (v) => {
  ...
  // For revenue, compute max for relative intensity
  ...
  return (
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="text-xs" style={{minWidth: allKeys.length * 40 + 100}}>
          <thead><tr>
            ...
            {data.map((row,ri)=>(
              <tr key={ri}>
                ...
                {allKeys.map(k=>{
                  ...
                  const bg = isEmpty ? '#F8F9FB' : !isPercent ? `rgba(26,60,245,${0.1+(v/maxVal)*0.8})` : color(v)
                  return (
                    <td key={k} className="px-0.5 py-0.5">
                      <div className="w-9 h-6 rounded flex items-center justify-center text-[9px] font-600 transition-all"
                        style={{background:bg, color:isEmpty?'transparent':textColor(v)}}>
                        {isEmpty ? '' : isPercent ? `${v}%` : fmt(v)}
                      </div>
                    </td>
                  )
                })}
...
function KpiCard({label,value,good,icon:Icon,accent}) {
  return (
    <div className={`bg-white rounded-xl border p-4 ${accent?'border-t-2 border-t-brand-600':'border-ink-200'}`}>
      {Icon&&<div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 ${good===true?'bg-green-50':good===false?'bg-red-50':'bg-brand-50'}`}><Icon size={15} className={good===true?'text-green-600':good===false?'text-red-500':'text-brand-600'}/></div>}
  ...
  return (
    <div className="h-56"><ResponsiveContainer width="100%" height="100%">
      <BarChart data={sorted} margin={{top:5,right:5,bottom:50,left:10}}>
        ...
        <Bar dataKey="value" radius={[3,3,0,0]}>{sorted.map((e,i)=><Cell key={i} fill={BRIDGE_COLORS[e.category]||'#CBD5E1'}/>)}</Bar>
      </BarChart>
...
export default function CommandCenter() {
  ...
  useEffect(()=>{
    ...
    setShowOpt(false);setShowKpi(false);setValidated(false)
  },[engine,columns])
  ...
  async function uploadFile(f) {
    ...
    } catch(e){setUploadErr(e.code==='ECONNABORTED'?'Timed out — try again in 10s.':`Could not read: ${e?.response?.data?.detail||e.message}`)}
    setUploading(false)
  }
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      if(engine==='cohort') {
        // Reuse existing cohort API — unchanged logic
        ...
        // Group-by cols as individual_cols
        const dimCols = cohortGroupBy.map(g=>{
          const key=g.toLowerCase()
          return Object.values(fieldMap).find(v=>v.toLowerCase().includes(key)) || fieldMap[key] || ''
        }).filter(Boolean)
        // Also add optional mapped cols
        const optCols=['product','region','channel'].map(k=>fieldMap[k]).filter(Boolean)
        const allCols=[...new Set([...dimCols,...optCols])]
        fd.append('individual_cols', JSON.stringify(allCols))
        ...
        fd.append('n_movers','30');fd.append('n_customers','10')
  ...
  // Cohort tabs when cohort engine
    ...
    <div className="flex h-screen bg-[#F6F7F9] overflow-hidden" style={{fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <aside className="w-72 bg-white border-r border-ink-200 flex flex-col overflow-hidden flex-shrink-0">
        ...
        <div className="px-3 py-2 border-b border-ink-100 bg-ink-50/50 flex-shrink-0">
          <div className="space-y-0.5">
            ...
            <Step n={4} label="Configure"       done={!!results} active={step3&&!results}/>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-4 border-b border-ink-100">
            ...
            {uploadErr&&<div className="mb-2 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-[10px] flex gap-1.5"><AlertCircle size={11} className="mt-0.5 flex-shrink-0"/><span>{uploadErr}</span></div>}
            <div onClick={()=>!uploading&&fileRef.current?.click()} className={`rounded-xl border-2 border-dashed p-4 text-center cursor-pointer transition-all ${file&&columns.length?'border-green-400 bg-green-50':uploading?'border-brand-300 bg-brand-50/30':'border-ink-200 hover:border-brand-300 bg-ink-50/30'}`}>
              <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={e=>{const f=e.target.files?.[0];if(f)uploadFile(f)}}/>
              {uploading?(<div><Loader2 size={18} className="text-brand-500 animate-spin mx-auto mb-1"/><div className="text-[11px] text-brand-700 font-600">Reading file...</div></div>)
              :file&&columns.length?(<div><CheckCircle size={18} className="text-green-500 mx-auto mb-1"/><div className="text-[11px] font-700 text-ink-900 truncate">{file.name}</div><div className="text-[10px] text-ink-500">{rowCount.toLocaleString()} rows · {columns.length} cols</div><button onClick={e=>{e.stopPropagation();fileRef.current?.click()}} className="text-[9px] text-brand-600 hover:underline mt-1 font-600">Change file</button></div>)
              :(<div><Upload size={18} className="text-ink-300 mx-auto mb-1"/><div className="text-[11px] text-ink-600 font-600">Click or drag CSV / Excel</div><div className="text-[10px] text-ink-400 mt-0.5">Alteryx output or raw revenue data</div></div>)}
            </div>
            ...
            <div className="p-4 border-b border-ink-100">
              ...
              <div className="space-y-1.5">
                {Object.entries(ENGINE_CONFIG).map(([id,ec])=>{
                  ...
                  return (
                    <button key={id} onClick={()=>setEngine(id)} className={`w-full flex items-start gap-2.5 p-3 rounded-xl border text-left transition-all ${active?'border-brand-400 bg-brand-50 shadow-sm':'border-ink-200 hover:border-ink-300 bg-white'}`}>
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${active?'bg-brand-600':'bg-ink-100'}`}><Icon size={13} className={active?'text-white':'text-ink-500'}/></div>
                      <div className="flex-1 min-w-0">
          ...
          {step2&&cfg&&(
            <div className="p-4 border-b border-ink-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[9px] text-ink-400">{cfg.required.filter(f=>!!fieldMap[f.key]).length}/{cfg.required.length} mapped</span>
              </div>

              {/* Required */}
              <div className="rounded-xl border border-ink-200 overflow-hidden mb-2">
                <div className="px-3 py-1.5 bg-ink-50 border-b border-ink-100"><span className="text-[9px] font-700 text-ink-500 uppercase tracking-widest">Required</span></div>
              ...
              {/* Optional */}
              <div className="rounded-xl border border-ink-200 overflow-hidden mb-2">
                <button onClick={()=>setShowOpt(v=>!v)} className="w-full px-3 py-1.5 bg-ink-50 flex items-center justify-between hover:bg-ink-100 transition-colors">
              ...
              <div className="rounded-xl border border-ink-200 overflow-hidden">
                <button onClick={()=>setShowKpi(v=>!v)} className="w-full px-3 py-1.5 bg-ink-50 flex items-center justify-between hover:bg-ink-100 transition-colors">
                ...
                {showKpi&&cfg.kpiLens.map(f=><FieldRow key={f.key} label={f.label} value={fieldMap[f.key]||''} columns={columns} onChange={v=>setFieldMap(m=>({...m,[f.key]:v}))} showError={false}/>)}
              </div>

              {validated&&Object.keys(errors).length>0&&<div className="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-[10px]">⚠ {Object.keys(errors).length} required field{Object.keys(errors).length>1?'s':''} not mapped</div>}
            </div>
          )}

          {/* STEP 4: Configure — dynamic per engine */}
          {step3&&(
            <div className="p-4">
              <div className="text-[9px] font-700 text-ink-400 uppercase tracking-widest mb-3">4. Configure</div>
              ...
              {engine==='cohort'&&(
                <div className="space-y-3">
                  ...
                  {/* Cohort Period */}
                  <div>
                    <div className="text-[10px] font-600 text-ink-600 mb-1.5">Cohort Period</div>
                    <div className="flex gap-1">
                      {COHORT_PERIODS.map(p=>(
                        <button key={p} onClick={()=>setCohortPeriod(p)} className={`flex-1 py-1 rounded text-[10px] font-600 border transition-all ${cohortPeriod===p?'bg-brand-600 text-white border-brand-600':'border-ink-200 text-ink-500'}`}>{p}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 mb-1.5">
                      {[['all','All Time'],['latest','Latest'],['fiscal_year','By FY']].map(([v,l])=>(
                        <button key={v} onClick={()=>setPeriodFilter(v)} className={`flex-1 py-1 rounded text-[10px] font-600 border transition-all ${periodFilter===v?'bg-brand-600 text-white border-brand-600':'border-ink-200 text-ink-500'}`}>{l}</button>
                      ))}
                    </div>
                    {periodFilter==='fiscal_year'&&(
                      <input value={selectedFY} onChange={e=>setSelectedFY(e.target.value)} placeholder="e.g. FY2024" className="w-full text-[11px] border border-ink-200 rounded-lg px-2 py-1.5 outline-none focus:border-brand-400"/>
                    )}
                  </div>

                  {/* Cohort Grouping */}
                  <div>
                    <div className="text-[10px] font-600 text-ink-600 mb-1.5">Cohort Grouping (Hierarchy)</div>
                    <div className="flex flex-wrap gap-1 mb-1.5">
                      {GROUPBY_OPTIONS.map(g=>(
                        <button key={g} onClick={()=>setCohortGroupBy(p=>p.includes(g)?p.filter(x=>x!==g):[...p,g])}
                          className={`px-2 py-0.5 rounded-full text-[10px] font-600 border transition-all ${cohortGroupBy.includes(g)?'bg-brand-600 text-white border-brand-600':'border-ink-200 text-ink-500 hover:border-ink-300'}`}>{g}</button>
                      ))}
                    </div>
                    {cohortGroupBy.length>0&&<div className="text-[9px] text-ink-400">Grouping by: {cohortGroupBy.join(' → ')}</div>}
                  </div>
                </div>
              )}
              ...
              {engine!=='cohort'&&(
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex gap-1">
                      {[1,3,6,12].map(lb=><button key={lb} onClick={()=>setLookbacks(p=>p.includes(lb)?p.filter(x=>x!==lb):[...p,lb])} className={`flex-1 py-1 rounded text-[10px] font-700 border transition-all ${lookbacks.includes(lb)?'bg-brand-600 text-white border-brand-600':'border-ink-200 text-ink-500'}`}>{lb}M</button>)}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] font-600 text-ink-600 mb-1">Revenue Units</div>
                    <div className="grid grid-cols-3 gap-1">
                      {[['raw','As-is'],['thousands','÷ 1K'],['millions','÷ 1M']].map(([v,l])=><button key={v} onClick={()=>setRevUnit(v)} className={`py-1 rounded text-[10px] font-600 border transition-all ${revenueUnit===v?'bg-brand-600 text-white border-brand-600':'border-ink-200 text-ink-500'}`}>{l}</button>)}
                    </div>
                  </div>
                  <div>
                    <div className="flex gap-1">
                      {['Annual','Quarter'].map(p=><button key={p} onClick={()=>setPeriod(p)} className={`flex-1 py-1 rounded text-[10px] font-700 border transition-all ${periodType===p?'bg-brand-600 text-white border-brand-600':'border-ink-200 text-ink-500'}`}>{p}</button>)}
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="p-4 border-t border-ink-100 flex-shrink-0">
          {runErr&&<div className="mb-2 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-[10px] flex gap-1.5"><AlertCircle size={10} className="mt-0.5 flex-shrink-0"/>{runErr}</div>}
          <button onClick={runAnalysis} disabled={!step1||!step2||running}
            ...
            className={`w-full flex items-center justify-center gap-2 font-700 text-sm py-3 rounded-xl transition-all ${canRun?'bg-brand-600 text-white hover:bg-brand-700':step3?'bg-brand-400 text-white cursor-not-allowed':'bg-ink-200 text-ink-400 cursor-not-allowed'}`}>
            {running?<Loader2 size={15} className="animate-spin"/>:<Play size={15}/>}
            {running?'Running analysis...':'Analyze Metrics'}
          </button>
          {running&&<UploadTimer active={running}/>}
          {results&&!running&&(isAdmin?(
            <button onClick={downloadCSV} className="w-full mt-2 flex items-center justify-center gap-2 border border-brand-300 text-brand-700 font-600 text-[11px] py-2 rounded-xl hover:bg-brand-50 transition-all"><Download size={12}/> Download Output CSV</button>
          ):(
            <button disabled className="w-full mt-2 flex items-center justify-center gap-2 border border-ink-200 text-ink-400 font-600 text-[11px] py-2 rounded-xl cursor-not-allowed"><Lock size={12}/> Download / Export <span className="ml-auto text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-700">Premium</span></button>
          ))}
          {!results&&!running&&<button disabled className="w-full mt-2 flex items-center justify-center gap-2 border border-ink-100 text-ink-300 font-600 text-[11px] py-2 rounded-xl cursor-not-allowed"><Lock size={12}/> Download / Export <span className="ml-auto text-[9px] bg-ink-100 text-ink-400 px-1.5 py-0.5 rounded-full font-700">Premium</span></button>}
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        <header className="h-14 bg-white border-b border-ink-200 flex items-center px-6 gap-4 flex-shrink-0">
          <div>
            <div className="text-sm font-700 text-ink-900">
              {engine==='cohort'?'Cohort Analytics':'Subscription Analytics'}
            </div>
            <div className="text-[10px] text-ink-400">
              {engine==='cohort'?'Retention heatmap · Revenue cohorts · Segmentation':'Real-time retention and revenue analytics'}
            </div>
          </div>
          ...
          {results&&(
            <div className="flex items-center gap-3">
              ...
              {isCohort&&results?.fiscal_years?.length>0&&(
                <select value={yearFilter} onChange={e=>setYearFilter(e.target.value)} className="text-[11px] border border-ink-200 rounded-lg px-2 py-1.5 bg-white text-ink-700 outline-none">
                  <option value="All">All Years</option>
                  {results.fiscal_years.map(fy=><option key={String(fy)} value={String(fy)}>{String(fy)}</option>)}
                </select>
              )}
        ...
        {!results&&(
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-lg">
              <div className="w-20 h-20 rounded-2xl bg-white border-2 border-ink-200 flex items-center justify-center mx-auto mb-5 shadow-sm"><BarChart3 size={32} className="text-brand-400"/></div>
              <h2 className="font-display text-2xl font-800 text-ink-900 mb-2">
                {engine==='cohort'?'Cohort Analytics':'Subscription Analytics'}
              </h2>
              <p className="text-ink-500 text-sm mb-6 leading-relaxed">
                {engine==='cohort'
                  ?'Upload your revenue data, map fields, configure cohort settings, and click Analyze to see retention heatmaps and revenue cohorts.'
                  :'Upload your Alteryx bridge output, select your analytics engine, map your fields, and click Analyze Metrics.'}
              </p>
              <div className="grid grid-cols-3 gap-3">
                {(engine==='cohort'?[
                  ...
                  <div key={i} className="bg-white rounded-xl border border-ink-200 p-4 text-left shadow-sm"><m.icon size={16} className="text-brand-500 mb-2"/><div className="text-xs font-700 text-ink-900 mb-1">{m.label}</div><div className="text-[10px] text-ink-400">{m.desc}</div></div>
                ))}
              </div>
            </div>
          </div>
        ...
        {results&&(
          <div className="flex-1 overflow-hidden flex flex-col">
              ...
              <div className="grid grid-cols-6 gap-3">
                {isCohort?(
                  <>
                    ...
                    <KpiCard label="Cohort Columns"   value={(results.summary?.cohort_cols?.length||0).toString()}         icon={Layers}/>
            ...
            <div className="px-6 border-b border-ink-200 bg-white flex flex-shrink-0">
              {TABS.map(tab=><button key={tab.id} onClick={()=>setActiveTab(tab.id)} className={`px-4 py-3 text-[12px] font-600 border-b-2 transition-all -mb-px ${activeTab===tab.id?'border-brand-600 text-brand-600':'border-transparent text-ink-500 hover:text-ink-700'}`}>{tab.label}</button>)}
            </div>
              ...
              {isCohort&&activeTab==='revenue_heatmap'&&(
                <div className="space-y-6">
                  {results.fy_summary?.length>0?(
                    <div className="bg-white rounded-xl border border-ink-200 p-5">
                      <div className="text-[10px] font-700 text-ink-400 uppercase tracking-widest mb-4">Revenue Summary by Fiscal Year</div>
                      ...
                      <div className="h-56 mt-6"><ResponsiveContainer width="100%" height="100%">
                  ...
                  ):<div className="bg-white rounded-xl border border-ink-200 p-10 text-center text-ink-400 text-sm">No fiscal year data. Map a Fiscal Year column.</div>}
                </div>
              )}

              {isCohort&&activeTab==='segmentation'&&(
