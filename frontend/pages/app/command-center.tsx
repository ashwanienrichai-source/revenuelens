...
export default function CommandCenter() {
  ...
  const [fuzzyGroupsData, setFuzzyGroupsData] = useState([])
  ...
  return (
    ...
    <div data-theme={themeMode} style={{display:'flex',height:'calc(100vh - 52px)',overflow:'hidden',position:'relative',background:T.bgPage,fontFamily:"'Inter',system-ui,sans-serif",color:T.textPrimary,colorScheme:(themeMode==='light'||themeMode==='light-red')?'light':'dark'}}>
      ...
      <button onClick={()=>setSidebarCollapsed(v=>!v)} title={sidebarCollapsed?'Expand':'Collapse'}
        style={{position:'absolute',top:'50%',transform:'translateY(-50%)',zIndex:50,left:sidebarCollapsed?0:256,width:16,height:40,background:T.bgSurface,border:`1px solid ${T.borderDefault}`,borderLeft:sidebarCollapsed?`1px solid ${T.borderDefault}`:'none',borderRadius:'0 6px 6px 0',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:T.textMuted,padding:0,transition:'left 0.25s ease'}}>
        <span style={{fontSize:9,fontWeight:700}}>{sidebarCollapsed?String.fromCharCode(187):String.fromCharCode(171)}</span>
      </button>

      <aside style={{width:sidebarCollapsed?0:256,minWidth:sidebarCollapsed?0:256,display:'flex',flexDirection:'column',flexShrink:0,borderRight:`1px solid ${T.borderDefault}`,background:T.bgSurface,overflow:'hidden',transition:'width 0.25s ease, min-width 0.25s ease'}}>
          ...
          {wizardStep!=='engine'&&(
            <div style={{padding:'10px 16px',borderBottom:`1px solid ${T.borderDefault}`}}>
              <div style={{display:'flex',alignItems:'center'}}>
                {['upload','map','quality','review'].map((id,i,arr)=>{
                  const idx=['upload','map','quality','review'].indexOf(wizardStep)
                  const done=i<idx,active=i===idx
                  const labels=['Upload','Map','Quality','Review']
                  return (
                    <div key={id} style={{display:'flex',alignItems:'center',flex:i<arr.length-1?1:'auto'}}>
                      <div style={{display:'flex',alignItems:'center',gap:3}}>
                        ...
                        <span style={{fontSize:9,fontWeight:active?600:400,color:active?T.textPrimary:T.textMuted,whiteSpace:'nowrap'}}>{labels[i]}</span>
                      </div>
          ...
          {wizardStep==='upload'&&(
            <div style={{padding:16}}>
              ...
              <input ref={wizardFileRef} type="file" accept=".csv,.xlsx,.xls" style={{display:'none'}}
                onChange={e=>{const f=e.target.files&&e.target.files[0];if(f){setWizardFile(f);setWizardRawRows([]);setQualityDone(false);setAppliedFuzzy(false);const reader=new FileReader();reader.onload=ev=>{const text=ev.target.result;const lns=text.split('\n').filter(l=>l.trim());if(!lns.length)return;const cols=lns[0].split(',').map(x=>x.trim().replace(/^"|"$/g,''));setColumns(cols);const rows=lns.slice(1).map(ln=>{const vals=ln.split(',').map(v=>v.trim().replace(/^"|"$/g,''));const row={};cols.forEach((col,i)=>{row[col]=vals[i]||''});return row});setWizardRawRows(rows);setRowCount(rows.length)};if(f.name.endsWith('.csv'))reader.readAsText(f)}}}/> 
              <div
                onClick={()=>wizardFileRef.current&&wizardFileRef.current.click()}
                onDragOver={e=>{e.preventDefault();setWizardDragging(true)}}
                onDragLeave={()=>setWizardDragging(false)}
                onDrop={e=>{e.preventDefault();setWizardDragging(false);const f=e.dataTransfer.files[0];if(f){setWizardFile(f);setColumns([]);setWizardRawRows([]);const r=new FileReader();r.onload=ev=>{const t=ev.target.result;const ls=t.split('\n').filter(l=>l.trim());if(!ls.length)return;const cols=ls[0].split(',').map(x=>x.trim().replace(/^"|"$/g,''));setColumns(cols);setWizardRawRows(ls.slice(1).map(ln=>{const vs=ln.split(',').map(v=>v.trim().replace(/^"|"$/g,''));const row={};cols.forEach((c2,i)=>{row[c2]=vs[i]||''});return row}))};if(f.name.endsWith('.csv'))r.readAsText(f)}}}
              ...
              <div style={{padding:12,border:`1px solid ${T.borderDefault}`,borderRadius:8,marginBottom:10,background:T.bgRaised}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:qualityDone&&totalCustomers>0?8:0}}>
                  ...
                  {!qualityDone
                    ?<button onClick={()=>{
                      ...
                      setTimeout(()=>{
                        const col=wizardMapping.customer
                        if(col&&wizardRawRows.length){
                          const names=[...new Set(wizardRawRows.map(r=>r[col]).filter(Boolean))]
                          setTotalCustomers(names.length)
                          const sorted=[...names].sort()
                          const groups=[],done2=new Set()
                          for(let i=0;i<sorted.length;i++){
                            if(done2.has(sorted[i]))continue
                            const grp=[sorted[i]];done2.add(sorted[i])
                            const na=sorted[i].toLowerCase().replace(/[.,_&()]+/g,' ').replace(/\s+/g,' ').trim()
                            for(let j=i+1;j<sorted.length;j++){
                              if(done2.has(sorted[j]))continue
                              const nb=sorted[j].toLowerCase().replace(/[.,_&()]+/g,' ').replace(/\s+/g,' ').trim()
                              if(na===nb||(na.length>3&&nb.startsWith(na.slice(0,Math.floor(na.length*0.8))))){grp.push(sorted[j]);done2.add(sorted[j])}
                            }
                            if(grp.length>1)groups.push(grp)
                          }
                          setFuzzyGroupsData(groups.map(g=>({names:g,canonical:g.reduce((a,b)=>a.length>=b.length?a:b)})))
                          const res={};groups.forEach(g=>g.forEach((n,i)=>{if(i>0)res[n]=g[0]}));setResolvedNames(res)
                        }
                        setQualityRunning(false);setQualityDone(true)
                      },800)
                    }} disabled={qualityRunning} style={{fontSize:9,fontWeight:700,padding:'4px 10px',borderRadius:5,border:'none',background:T.selectionBg,color:T.growth,cursor:'pointer',display:'flex',alignItems:'center',gap:4}}>
                      {qualityRunning?<><Loader2 size={9} style={{animation:'spin 1s linear infinite'}}/>Running</>:'Run Check'}
                    </button>
                    :!appliedFuzzy&&fuzzyGroupsData.length>0
                    ?<button onClick={()=>{
                      const col=wizardMapping.customer
                      if(!col)return
                      setWizardRawRows(prev=>prev.map(row=>({...row,[col]:resolvedNames[row[col]]||row[col]})))
                      setAppliedFuzzy(true)
                    }} style={{fontSize:9,fontWeight:700,padding:'4px 10px',borderRadius:5,border:'none',background:T.growth+'20',color:T.growth,cursor:'pointer'}}>Apply</button>
                ...
                {qualityDone&&(
                  <div style={{display:'flex',gap:16}}>
                    ...
                    {fuzzyGroupsData.length>0&&!appliedFuzzy&&<div><div style={{fontSize:8,color:T.textMuted,textTransform:'uppercase',letterSpacing:'0.08em'}}>Issues</div><div style={{fontSize:18,fontWeight:700,color:T.warning}}>{fuzzyGroupsData.reduce((s,g)=>s+g.names.length,0)}</div></div>}
                    {appliedFuzzy&&<div style={{fontSize:9,color:T.growth,fontWeight:600,display:'flex',alignItems:'center',gap:3}}><CheckCircle size={10}/>Consolidated</div>}
                  </div>
                )}
              </div>
            ...
            <div style={{padding:16}}>
              ...
              <div style={{padding:12,border:`1px solid ${T.borderDefault}`,borderRadius:8,background:T.bgRaised,marginBottom:10}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                  <div><div style={{fontSize:8,color:T.textMuted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:2}}>File</div><div style={{fontSize:10,fontWeight:600,color:T.textPrimary,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{wizardFile&&wizardFile.name}</div></div>
                  ...
                  <div><div style={{fontSize:8,color:T.textMuted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:2}}>Cols</div><div style={{fontSize:10,fontWeight:600,color:T.textPrimary}}>{columns.length}</div></div>
              ...
              <div style={{padding:'7px 10px',border:`1px solid ${qualityDone?T.growth+'40':T.borderDefault}`,borderRadius:6,background:qualityDone?T.growth+'08':T.bgRaised,marginBottom:12,fontSize:9,display:'flex',alignItems:'center',gap:6}}>
                ...
                <span style={{color:qualityDone?T.growth:T.textMuted}}>{qualityDone?'Quality checked':'Quality checks not run'}</span>
              </div>
              <div style={{display:'flex',gap:6}}>
                ...
                <button onClick={()=>{
                  ...
                  const fm={};Object.entries(wizardMapping).forEach(([k,v])=>{if(v&&v!=='None')fm[k]=v})
                  setFile(wizardFile);setFieldMap(fm);setUploadDatasetType(wizardDatasetType);setWizardStep('engine')
                }} style={{flex:2,padding:'9px',borderRadius:8,border:'none',background:T.selectionBg,color:T.growth,fontSize:11,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
            ...
            <div style={{padding:16,borderBottom:`1px solid ${T.borderDefault}`}}>
              {file&&(
                <div style={{display:'flex',alignItems:'center',gap:8,padding:'7px 10px',borderRadius:6,background:T.bgRaised,border:`1px solid ${T.borderDefault}`,marginBottom:12}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:10,fontWeight:600,color:T.textPrimary,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{wizardFile&&wizardFile.name}</div>
                    <div style={{fontSize:9,color:T.textSecondary}}>{wizardRawRows.length>0?wizardRawRows.length.toLocaleString()+' rows':''} {columns.length} cols{uploadDatasetType?' · '+uploadDatasetType:''}</div>
                  </div>
                  <button onClick={()=>{setWizardStep('upload');setResults(null);setFile(null);setColumns([]);setEngine(null);setFieldMap({})}} style={{fontSize:9,color:T.textMuted,background:'transparent',border:'none',cursor:'pointer'}}>&#8635;</button>
                </div>
              )}
