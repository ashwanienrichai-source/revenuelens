        ...
        <div key={s} className="flex items-center">
          <div className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-600 ${
            ...
            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-700 ${
              i + 1 < step  ? 'bg-brand-200 text-brand-700' :
              i + 1 === step ? 'bg-white/25 text-white' : 'bg-ink-200 text-ink-500'
            }`}>{i + 1}</div>
            {s}
          </div>
...
function Heatmap({ data, title }: { data: any[]; title: string }) {
  ...
  const color = (v: number | null) => {
    ...
    if (v >= 90) return '#1A3CF5'
    if (v >= 70) return '#3D5EFF'
    if (v >= 50) return '#6285FF'
    if (v >= 30) return '#BACBFF'
    if (v >= 10) return '#D9E4FF'
    return '#EEF3FF'
  }
  return (
    <div>
...
export default function CohortPage() {
  ...
  useEffect(() => {
    ...
    // Wake up Render API on page load
  ...
  async function readFileColumns(f: File) {
    ...
    // Try with timeout — if it takes > 5s, show "waking up" message
    ...
    try {
      ...
      const find = (kw: string[]) =>
        data.columns.find((_: string, i: number) => kw.some(k => cols[i].includes(k))) || ''
      ...
      const fy = data.columns.find((_: string, i: number) =>
        cols[i].includes('fiscal') || cols[i] === 'fy') || 'None'
      setFiscalCol(fy)
    } catch (e: any) {
      clearTimeout(wakeTimer)
      setApiWaking(false)
      if (e.code === 'ECONNABORTED' || e.message?.includes('timeout')) {
        setError('API is starting up — please try uploading again in 10 seconds.')
      } else {
        setError(`Could not read file: ${e?.response?.data?.detail || e.message || 'Unknown error'}`)
      }
    }
  ...
  async function runAnalysis() {
    if (!file || !metric || !customerCol || !dateCol) {
    ...
    try {
      ...
      fd.append('individual_cols', JSON.stringify(
        dimensionMode === 'single' ? individualCols.filter(c => c && c !== 'None') : []
      ))
      fd.append('hierarchies', JSON.stringify(
        dimensionMode === 'multi' ? hierarchies.filter(h => h.some(c => c && c !== 'None')) : []
      ))
  ...
  function downloadCSV() {
    ...
    const csv = [keys.join(','), ...results.output.map((r: any) =>
      keys.map((k: string) => r[k] ?? '').join(',')
    )].join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    a.download = 'cohort_output.csv'
    a.click()
  }
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-ink-400 text-sm ml-12">
              Size Cohorts · Percentile Cohorts · Revenue Cohorts · Retention Heatmap
            </p>
          </div>
          {results && (
            <div className="flex items-center gap-3">
              <button onClick={() => { setStep(1); setResults(null); setFile(null); setColumns([]) }}
        ...
        {step === 1 && (
          <div className="max-w-2xl">
            ...
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-2.5 text-xs text-blue-800">
              ...
              <span>
                <strong>First upload may take 30-60 seconds</strong> — the analytics engine wakes up on first use.
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`rounded-2xl border-2 border-dashed p-12 text-center transition-all mb-4 ${
                file && columns.length > 0
                  ? 'border-green-400 bg-green-50'
                  : 'border-ink-200 hover:border-brand-300 bg-ink-50/50 cursor-pointer'
              }`}
            ...
            >
              ...
              {loadingCols ? (
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="text-ink-700 text-sm font-600">
                    {apiWaking ? '⏳ API is waking up — please wait 30-60 seconds...' : 'Reading your file...'}
                  </div>
                  {apiWaking && (
                    <div className="text-ink-400 text-xs">This only happens on first use each day</div>
                  )}
                </div>
              ) : file && columns.length > 0 ? (
                <div>
                ...
                <div>
                  ...
                  <div className="font-600 text-ink-700 text-sm mb-1">
            ...
            {error && (
              <div className="mb-4 p-3.5 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm flex items-center gap-2">
                ...
                {error.includes('try uploading again') && (
                  <button onClick={() => file && readFileColumns(file)}
                    className="ml-auto text-xs font-600 underline">Retry</button>
                )}
              </div>
            )}
            ...
            {columns.length > 0 && (
              <>
                <div className="card p-4 mb-4">
                  <div className="text-xs font-700 text-ink-400 uppercase tracking-widest mb-3">
                    Detected columns ({columns.length})
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {columns.map(c => (
                      <span key={c} className="px-2.5 py-1 bg-ink-50 border border-ink-200 rounded-lg text-xs text-ink-600 font-500">
                        {c}
                      </span>
                    ))}
                  </div>
                ...
                <button onClick={() => setStep(2)} className="btn-primary">
                  Configure Analysis <ChevronRight size={14} />
                </button>
              </>
            )}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {COHORT_TYPES.map(ct => (
                    <label key={ct.id} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                      ...
                      <input type="checkbox" checked={cohortTypes.includes(ct.id)}
                        className="mt-0.5 accent-brand-600"
                        onChange={e => setCohortTypes(prev =>
                          e.target.checked ? [...prev, ct.id] : prev.filter(x => x !== ct.id)
                        )} />
                      <div>
              ...
              <div className="card p-5">
                ...
                {periodFilter === 'fiscal_year' && (
                  <input className="input-field text-sm" placeholder="e.g. FY2024"
                    ...
                    {individualCols.map((col, i) => (
                      <div key={i} className="flex gap-2">
                        ...
                        <button onClick={() => setIndividualCols(prev => prev.filter((_, j) => j !== i))}
                          className="p-1.5 text-ink-400 hover:text-red-500 rounded">
                          <X size={14} />
                        </button>
                      </div>
                    ))}
                      ...
                      <div key={hi} className="p-3 bg-ink-50 rounded-xl border border-ink-200">
                        ...
                        {hier.map((col, ci) => (
                          <div key={ci} className="flex gap-2 mb-1.5">
                            <select value={col}
                              onChange={e => {
                                const n = hierarchies.map((h, j) =>
                                  j === hi ? h.map((c, k) => k === ci ? e.target.value : c) : h)
                                setHierarchies(n)
                              }}
                              className="input-field text-sm py-1.5 flex-1">
                              <option value="">— Select —</option>
                        ...
                        <button onClick={() => setHierarchies(prev => prev.map((h, j) => j === hi ? [...h, ''] : h))}
                          className="text-xs text-brand-600 font-600 flex items-center gap-1">
            ...
            <div className="lg:col-span-2 flex items-center gap-4">
              ...
              <button onClick={runAnalysis}
                disabled={running || !metric || !customerCol || !dateCol}
                className="btn-primary disabled:opacity-40 disabled:cursor-not-allowed">
                {running ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
                {running ? 'Running analysis...' : 'Run Cohort Analysis'}
              </button>
              {running && (
                <div className="text-ink-400 text-xs">This may take 30-60 seconds on first run</div>
              )}
            </div>
          </div>
        )}
          ...
          <div>
            ...
            {activeTab === 'summary' && (
              <div className="space-y-6">
                ...
                {!results.fy_summary?.length && (
                  <div className="card p-8 text-center text-ink-400 text-sm">
                    No fiscal year summary available. Map a Fiscal Year column to see this breakdown.
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'heatmap' && (
              <div className="space-y-6">
                {results.heatmap?.length > 0 ? (
                  <div className="card p-6">
                    <Heatmap data={results.heatmap} title="Customer Count — Cohort Heatmap" />
                  </div>
                ) : (
                  <div className="card p-10 text-center text-ink-400 text-sm">
                    No heatmap data. Make sure your Date column has valid dates.
                  </div>
                )}
                {results.retention?.length > 0 && (
                  <div className="card p-6">
                    <Heatmap data={results.retention} title="Retention Rate % by Cohort" />
                  </div>
                )}
              </div>
            )}
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={results.segmentation}
                            dataKey={Object.keys(results.segmentation[0]).find(k => k !== 'segment') || ''}
                            nameKey="segment" cx="50%" cy="50%" outerRadius={90}
                            label={({ segment, percent }: any) => `${segment} ${(percent * 100).toFixed(0)}%`}>
                            {results.segmentation.map((_: any, i: number) => (
                          ...
                          <div key={i} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-xs text-ink-400 w-12 text-right">
                                {total > 0 ? `${((r[vk] / total) * 100).toFixed(1)}%` : '—'}
                              </span>
                            </div>
                          </div>
                        )
            ...
            {activeTab === 'output' && (
              <div className="card overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-ink-100 bg-ink-50/50">
                  ...
                  {isAdmin ? (
                    <button onClick={downloadCSV} className="btn-primary text-xs py-1.5 px-3">
                      <Download size={11} /> Download CSV
                    </button>
                  ) : (
                    <button onClick={() => router.push('/dashboard/upgrade')}
                      className="flex items-center gap-1.5 text-xs font-600 text-ink-500 bg-ink-100 px-3 py-1.5 rounded-lg hover:bg-ink-200">
